...
Distinctive + refined is the goal. The specific shape depends on the brand, not on the brand's category. A category ("restaurant", "dev tool", "magazine", "fintech") is not a recipe; treating it as one is the first-order reflex described in the [category-reflex check](design-guidance.md#the-ai-slop-test); read that check when choosing the brand direction.
